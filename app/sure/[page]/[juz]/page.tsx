...
export default function page({ params }: { params: { page: number, juz: number } }) {
    ...
    useEffect(() => {
        fetch(`https://quran.rafed.net/api/get_pack/${params.page}/`)
            .then((res) => res.json())
    ...
    return (
        <div className="relative flex justify-around flex-col">
                ...
                <nav className="border-black  bg-white text-typography shadow-xl pr-3  ">
                    <div className="max-w-screen-full flex flex-wrap items-center justify-between mx-auto ">
                        ...
                        <div className="flex justify-between items-center flex-row w-1/2">
                            ...
                            <div className="flex items-center space-x-3 pr-3 rtl:space-x-reverse lg:pr-7 xl:pr-7">
                                <Link href='./' className='Soreh  h-20 w-20 text-typography flex justify-evenly items-center '>{NameData?.sura_name}</Link>
                            </div>
                            <div className="flex items-center space-x-3 pr-3 rtl:space-x-reverse lg:pr-7 xl:pr-7">
            ...
            <div onClick={() => {
                ...
                setShowAuther(false)
            }} className={`h-96 bg-slate-200 text-typography overflow-auto text-${textSize}xl  w-full   md:w-4/5  lg:w-4/5 mt-10 mr-auto ml-auto`}>
                {/* برای جایگذاری قرآن  */}
            ...
            {menuBar ? '' : <footer className="w-full p-3 bg-white fixed bottom-0  flex justify-between items-center  mt-9 h-11  z-10 border-black  shadow-xl pr-3 ">
                ...
                <span><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-7 font-bold cursor-pointer">
