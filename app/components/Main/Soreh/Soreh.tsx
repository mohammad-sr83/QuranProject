...
export default function Soreh() {
    ...
    return (
        <div className="pt-10 pr-4 pl-4 font-[Quran] ">
                ...
                {show ? (
                    secend_deta.map((items) => (
                        <div key={items.sura} className="flex justify-center  items-center m-8 lg:m-12 md:m-12 ">
                            ...
                            <Link href={`./sure/${items.sura}/1`}  className="text-2xl flex font-[Quran] text-black" onClick={()=>setCookie('lastSure',`${items.sura}`)}>{items.sura_name}</Link>
                        </div>
                    ))
                ) : (
                    deta.map((items) => (
                        <div key={items.sura} className="flex justify-center  items-center m-8 ">
                            ...
                            <Link href={`./sure/${items.sura}/1`} className="text-2xl flex  font-[Quran]" onClick={()=>setCookie('lastSure',`${items.sura}`)}>{items.sura_name}</Link>
                        </div>
                    ))
                )}
            </div>
