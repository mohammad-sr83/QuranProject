            ...
            <div className='flex justify-between flex-wrap items-center  lg:gap-12 md:gap-12 '>
                {show ? (
                    secend_deta.map((items) => (
                        <div key={items.sura} className="flex justify-center  items-center m-10 lg:m-12 md:m-12 ">
                            <div className="number_Soreh h-10 w-10 flex justify-center items-center">{items.sura}</div>
                            <Link href={`./sure/${items.sura}/${items.sura}`} className="text-2xl flex ">{items.sura_name}</Link>
                        </div>
                    ))
                ) : (
                    deta.map((items) => (
                        <div key={items.sura} className="flex justify-center  items-center m-10 ">
                            <div className="number_Soreh h-10 w-10 flex justify-center items-center">{items.sura}</div>
                            <Link href={`./sure/${items.sura}/${items.sura}`} className="text-2xl flex ">{items.sura_name}</Link>
                        </div>
                    ))
